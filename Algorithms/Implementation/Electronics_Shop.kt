...
fun getMoneySpent(keyboards: Array<Int>, drives: Array<Int>, b: Int): Int {
    ...
    var selectableDevices = mutableListOf<Int>()

    for (i in keyboards.filter { it -> it < b }) {
        for (j in drives.filter { it -> it < b }) {
            selectableDevices.add(i + j)
        }
    }

    var result = selectableDevices.filter { it -> it <= b }.max()
